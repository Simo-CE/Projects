...
function M(e) {
	...
	if (Object.assign(a, {
			...
			isScrolling: void 0,
			startMoving: void 0
		}), l.startX = g, l.startY = v, a.touchStartTime = u(), t.allowClick = !0, t.updateSize(), t.swipeDirection = void 0, o.threshold > 0 && (a.allowThresholdMove = !1), "touchstart" !== p.type) {
		let e = !0;
...
function P(e) {
	...
	let w = !0,
		T = r.resistanceRatio;
	if (r.touchReleaseOnEdges && (T = 0), v > 0 && n.currentTranslate > s.minTranslate() ? (w = !1, r.resistance && (n.currentTranslate = s.minTranslate() - 1 + (-s.minTranslate() + n.startTranslate + v) ** T)) : v < 0 && n.currentTranslate < s.maxTranslate() && (w = !1, r.resistance && (n.currentTranslate = s.maxTranslate() + 1 - (s.maxTranslate() - n.startTranslate - v) ** T)), w && (c.preventedByNestedSwiper = !0), !s.allowSlideNext && "next" === s.swipeDirection && n.currentTranslate < n.startTranslate && (n.currentTranslate = n.startTranslate), !s.allowSlidePrev && "prev" === s.swipeDirection && n.currentTranslate > n.startTranslate && (n.currentTranslate = n.startTranslate), s.allowSlidePrev || s.allowSlideNext || (n.currentTranslate = n.startTranslate), r.threshold > 0) {
...
function k(e) {
	...
	for (let p = 0; p < a.length; p += p < i.slidesPerGroupSkip ? 1 : i.slidesPerGroup) {
	...
	if (c > i.longSwipesMs) {
		...
		"next" === t.swipeDirection && (w >= i.longSwipesRatio ? t.slideTo(i.rewind && t.isEnd ? g : f + T) : t.slideTo(f)), "prev" === t.swipeDirection && (w > 1 - i.longSwipesRatio ? t.slideTo(f + T) : null !== v && w < 0 && Math.abs(w) > i.longSwipesRatio ? t.slideTo(v) : t.slideTo(f))
	} else {
		if (!i.shortSwipes) return void t.slideTo(t.activeIndex);
		t.navigation && (l.target === t.navigation.nextEl || l.target === t.navigation.prevEl) ? l.target === t.navigation.nextEl ? t.slideTo(f + T) : t.slideTo(f) : ("next" === t.swipeDirection && t.slideTo(null !== g ? g : f + T), "prev" === t.swipeDirection && t.slideTo(null !== v ? v : f))
	}
}
...
const z = (e, t) => {
	...
	(n.preventClicks || n.preventClicksPropagation) && a[p]("click", e.onClick, !0), n.cssMode && o[p]("scroll", e.onScroll), n.updateOnWindowResize ? e[u](l.ios || l.android ? "resize orientationchange observerUpdate" : "resize observerUpdate", L, !0) : e[u]("observerUpdate", L, !0)
};
...
const _ = {
		...
		update: {
			...
			updateSlides: function() {
				...
				for (let v = 0; v < p; v += 1) {
					C = 0;
					const n = c.eq(v);
					if (y && e.grid.updateSlide(v, n, p, t), "none" !== n.css("display")) {
						...
						} else C = (r - (i.slidesPerView - 1) * b) / i.slidesPerView, i.roundLengths && (C = Math.floor(C)), c[v] && (c[v].style[t("width")] = `${C}px`);
						c[v] && (c[v].swiperSlideSize = C), f.push(C), i.centeredSlides ? (S = S + C / 2 + x / 2 + b, 0 === x && 0 !== v && (S = S - r / 2 - b), 0 === v && (S = S - r / 2 - b), Math.abs(S) < .001 && (S = 0), i.roundLengths && (S = Math.floor(S)), E % i.slidesPerGroup == 0 && u.push(S), h.push(S)) : (i.roundLengths && (S = Math.floor(S)), (E - Math.min(e.params.slidesPerGroupSkip, E)) % e.params.slidesPerGroup == 0 && u.push(S), h.push(S), S = S + C + b), e.virtualSize += C + b, x = C, E += 1
					}
				}
				...
				if (p !== d && e.emit("slidesLengthChange"), u.length !== w && (e.params.watchOverflow && e.checkOverflow(), e.emit("snapGridLengthChange")), h.length !== T && e.emit("slidesGridLengthChange"), i.watchSlidesProgress && e.updateSlidesOffset(), !(l || i.cssMode || "slide" !== i.effect && "fade" !== i.effect)) {
			...
			updateSlidesProgress: function(e) {
				...
				for (let o = 0; o < i.length; o += 1) {
					...
					(p >= 0 && p < t.size - 1 || u > 1 && u <= t.size || p <= 0 && u >= t.size) && (t.visibleSlides.push(e), t.visibleSlidesIndexes.push(o), i.eq(o).addClass(s.slideVisibleClass)), e.progress = n ? -d : d, e.originalProgress = n ? -c : c
				}
			...
			slideTo: function(e, t, s, i, n) {
				...
				if (r.updateProgress(v), o.normalizeSlideIndex)
					for (let w = 0; w < d.length; w += 1) {
						...
						void 0 !== d[w + 1] ? e >= t && e < s - (s - t) / 2 ? a = w : e >= t && e < s && (a = w + 1) : e >= t && (a = w)
					}
				...
				let T;
				if (T = a > p ? "next" : a < p ? "prev" : "reset", u && -v === r.translate || !u && v === r.translate) return r.updateActiveIndex(a), o.autoHeight && r.updateAutoHeight(), r.updateSlidesClasses(), "slide" !== o.effect && r.setTranslate(v), "reset" !== T && (r.transitionStart(s, T), r.transitionEnd(s, T)), !1;
				if (o.cssMode) {
					const e = r.isHorizontal(),
						s = u ? v : -v;
					if (0 === t) {
				...
				return r.setTransition(t), r.setTranslate(v), r.updateActiveIndex(a), r.updateSlidesClasses(), r.emit("beforeTransitionStart", t, i), r.transitionStart(s, T), 0 === t ? r.transitionEnd(s, T) : r.animating || (r.animating = !0, r.onSlideToWrapperTransitionEnd || (r.onSlideToWrapperTransitionEnd = function(e) {
		...
		loop: {
			loopCreate: function() {
				...
				for (let i = 0; i < l.length; i += 1) r.append(d(l[i].cloneNode(!0)).addClass(s.slideDuplicateClass));
				for (let i = o.length - 1; i >= 0; i -= 1) r.prepend(d(o[i].cloneNode(!0)).addClass(s.slideDuplicateClass))
			},
			loopFix: function() {
				const e = this;
		...
		events: {
			attachEvents: function() {
				...
				e.onTouchStart = M.bind(e), e.onTouchMove = P.bind(e), e.onTouchEnd = k.bind(e), s.cssMode && (e.onScroll = $.bind(e)), e.onClick = O.bind(e), n.touch && !A && (t.addEventListener("touchstart", D), A = !0), z(e, "on")
			},
...
class H {
	constructor() {
		...
		const r = this;
		r.__swiper__ = !0, r.support = x(), r.device = E({
			userAgent: t.userAgent
		}), r.browser = y(), r.eventsListeners = {}, r.eventsAnyListeners = [], r.modules = [...r.__modules__], t.modules && Array.isArray(t.modules) && r.modules.push(...t.modules);
		const a = {};
		r.modules.forEach((e => {
			e({
				swiper: r,
...
function j(e) {
	...
	i("init", (() => {
		!t.params.mousewheel.enabled && t.params.cssMode && b(), t.params.mousewheel.enabled && T()
	})), i("destroy", (() => {
		t.params.cssMode && T(), t.mousewheel.enabled && b()
